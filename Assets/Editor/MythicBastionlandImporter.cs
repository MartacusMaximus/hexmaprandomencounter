...
public static class MythicBastionlandImporter
{
    ...
    public static void ImportPdfContent()
    {
        ...
        foreach (var knight in payload.knights ?? Array.Empty<KnightRecord>())
        {
            ...
            asset.randomFlavorTableTitle = knight.randomFlavorTableTitle;
            asset.randomFlavorTableRows = knight.randomFlavorTableRows?.ToList() ?? new List<string>();
            asset.personHook = knight.personHook;
            asset.objectHook = knight.objectHook;
            asset.beastHook = knight.beastHook;
            asset.stateHook = knight.stateHook;
            asset.themeHook = knight.themeHook;
        ...
        foreach (var myth in payload.myths ?? Array.Empty<MythRecord>())
        {
            ...
            asset.flavorTable = new MythFlavorTable
            {
                title = myth.flavorTableTitle,
                rows = myth.flavorTableRows?.ToList() ?? new List<string>()
            };
            asset.dwelling = myth.dwelling;
            asset.sanctum = myth.sanctum;
            asset.monument = myth.monument;
            asset.hazard = myth.hazard;
            asset.curse = myth.curse;
            asset.ruin = myth.ruin;
        ...
        contentLibrary.seers = AssetDatabase.FindAssets("t:SeerDefinitionSO", new[] { SeerFolder })
            .Select(AssetDatabase.GUIDToAssetPath)
            .Select(AssetDatabase.LoadAssetAtPath<SeerDefinitionSO>)
            .Where(asset => asset != null)
            .OrderBy(asset => asset.seerName)
            .ToList();
        contentLibrary.abilities = AssetDatabase.FindAssets("t:AbilitySO", new[] { AbilityFolder })
            .Select(AssetDatabase.GUIDToAssetPath)
            .Select(AssetDatabase.LoadAssetAtPath<AbilitySO>)
            .Where(asset => asset != null)
            .OrderBy(asset => asset.abilityName)
            .ToList();
        contentLibrary.equipment = AssetDatabase.FindAssets("t:EquipmentData", new[] { EquipmentFolder })
            .Select(AssetDatabase.GUIDToAssetPath)
            .Select(AssetDatabase.LoadAssetAtPath<EquipmentData>)
            .Where(asset => asset != null)
            .OrderBy(asset => asset.itemName)
            .ToList();
        contentLibrary.knights = AssetDatabase.FindAssets("t:KnightDefinitionSO", new[] { KnightFolder })
            .Select(AssetDatabase.GUIDToAssetPath)
            .Select(AssetDatabase.LoadAssetAtPath<KnightDefinitionSO>)
            .Where(asset => asset != null)
            .OrderBy(asset => asset.pageNumber)
            .ToList();
        contentLibrary.myths = AssetDatabase.FindAssets("t:MythSO", new[] { MythFolder })
            .Select(AssetDatabase.GUIDToAssetPath)
            .Select(AssetDatabase.LoadAssetAtPath<MythSO>)
            .Where(asset => asset != null)
            .OrderBy(asset => asset.pageNumber)
            .ToList();
    ...
    private sealed class KnightRecord
    {
        ...
        public string randomFlavorTableTitle;
        public string[] randomFlavorTableRows;
    ...
    private sealed class MythRecord
    {
        ...
        public string flavorTableTitle;
        public string[] flavorTableRows;
